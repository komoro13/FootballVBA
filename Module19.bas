...
Sub step_10(ByVal FileName As String, ByVal Row_ As Long)
...
Worksheets("Step 10").Cells.ClearContents
...
Worksheets(FileName).range("A" & j).EntireRow.Copy ThisWorkbook.Sheets("Step 10").range("A" & CStr(x))
x = x + 1
End If
...
Next j
 ...
 'MsgBox Rows.Count
Sheets("Step 10").Select
range("A1:AX" & Rows.count).Sort key1:=range("AM1"), Order1:=xlDescending, Header:=xlNo
Sheets("Step 10").Select
'MsgBox "Copied!"
End Sub
