...
Sub step_2(ByVal FileName As String, ByVal Row_ As Long)
...
Worksheets("Step 2").Cells.ClearContents
...
Worksheets(FileName).range("A" & j).EntireRow.Copy ThisWorkbook.Sheets("Step 2").range("A" & CStr(x))
x = x + 1
End If
...
range("A1:AX" & Rows.count).Sort key1:=range("AR1"), Order1:=xlDescending, Header:=xlNo
Sheets("Step 2").Select
'MsgBox "Copied!"
End Sub
